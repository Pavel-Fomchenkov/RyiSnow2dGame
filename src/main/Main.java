...
//  При полном инвентаре попытка подобрать предмет заполняет текстовый лог сообщениями о невозможности подбора
//  Хотелось бы сделать только одно сообщение
//  Поиск пути не учитывает закрытые двери, из-за чего npc бесконечно упирается в закрытую дверь